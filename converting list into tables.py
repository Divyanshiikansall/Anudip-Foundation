...
def print_table(student): #function is used to prinst the list given by user in table format
     for row in student:
        print("| {:<10} | {:<10} | {:<10} | {:<5} | {:<5} | {:<10} | {:<5} | {:<10} | {:<10} | {:<5} |".format(*row))
def query_1(student):  #function for query 1
    print("\nQuery 1: Names of students whose marks in English are greater than 50")
    for row in student[1:]:  
        if row[6] > 50:  
            print(row[1])
def query_2(student):  #function for query 2
...
def query_3(student):  #function for query 3
